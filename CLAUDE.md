...
- **Repo: initialised.** Next.js 15 App Router + TypeScript, Supabase migrations with
  RLS, agent runtime, WhatsApp webhook, owner portal, 96 tests passing.
...
- Supabase Auth sign-in screen. The portal reads through the RLS-protected user
  client and shows a setup notice when there is no session; wiring the auth UI is
  the next short task.
...
1. Supabase Auth sign-in and the `users` bootstrap for a new tenant.
2. Run the local stack end to end: `supabase start`, `supabase db reset`, ngrok tunnel,
   Meta test number, a real inbound message through to a booking.
3. Self-serve onboarding: turn `/config/tenants/*.json` into an apply script so a second
   clinic goes live without touching the database by hand.
